...
async function main() {
  ...
  const template1 = await prisma.template.upsert({
    ...
    create: {
      ...
      description:
        "Form for client consultations for Botox and Dermal Filler treatments.",
      type: "Consultation Form",
      filePath: path.join(
        __dirname,
        "../36e49c4d-5236-45a0-ba04-9e0c1aedd38c.pdf",
      ),
    },
  });
  ...
  const template2 = await prisma.template.upsert({
    ...
    create: {
      ...
      description:
        "Certificate for Advanced Level 6 Diploma in Botox, Dermal Fillers, and Complication Management.",
      type: "Certificate",
      filePath: path.join(
        __dirname,
        "../ee503724-8cca-4acd-b8b1-d754c380f179.pdf",
      ),
    },
  });
