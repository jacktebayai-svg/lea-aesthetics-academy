export class MASClient {
  ...
  async availability(params: {
    ...
    const r = await fetch(
      `${this.cfg.baseUrl}/v1/availability?location_id=${params.locationId}&service_id=${params.serviceId}&from=${encodeURIComponent(params.from)}&to=${encodeURIComponent(params.to)}`,
  ...
  async createAppointment(payload: any) {
    const r = await fetch(`${this.cfg.baseUrl}/v1/appointments`, {
      method: "POST",
      headers: this.headers(),
  ...
  async generateDocument(payload: any) {
    const r = await fetch(`${this.cfg.baseUrl}/v1/documents/generate`, {
      method: "POST",
      headers: this.headers(),
      body: JSON.stringify(payload),
    });
