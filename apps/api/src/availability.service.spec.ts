...
describe('AvailabilityService', () => {
  it('computes slots for 2 hours window with 30min service', async () => {
    ...
    (svc as any).prisma = { service: { findUnique: async () => ({ durationMin: 30, buffers: { beforeMin: 0, afterMin: 0 } }) } };
    ...
    const slots = await svc.getSlots({ tenantId: 't', locationId: 'l', serviceId: 's', from, to });
