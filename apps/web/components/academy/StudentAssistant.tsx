...
export default function StudentAssistant() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const assistantPlaceholder: ChatMessage = { id: assistantMessageId, role: 'assistant', text: '' };
    ...
    try {
      ...
      setMessages(prev => prev.map(msg => 
          msg.id === assistantMessageId ? { ...msg, text: `Error: ${errorMessage}` } : msg
      ));
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Card className="flex flex-col h-[600px]">
      <div className="flex items-center mb-4">
        ...
        <h2 className="text-xl font-playfair font-semibold text-slate ml-2">Student Assistant</h2>
      </div>
      <div className="flex-grow overflow-y-auto pr-2 space-y-4 bg-ivory p-4 rounded-xl border border-smoke" aria-live="polite">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex items-start gap-2.5 ${msg.role === 'user' ? 'justify-end' : ''}`}>
            {msg.role === 'assistant' && <div className="w-8 h-8 rounded-full bg-gold/20 flex-shrink-0 flex items-center justify-center"><Icon name="message-circle" className="w-5 h-5 text-gold"/></div>}
            <div className={`p-3 rounded-xl max-w-xs md:max-w-sm ${msg.role === 'user' ? 'bg-slate text-ivory rounded-br-none' : 'bg-smoke/60 text-charcoal rounded-bl-none'}`}>
              <p className={`text-sm ${(isLoading && msg.role === 'assistant' && msg.text === '') ? 'animate-pulse' : ''}`} style={{ whiteSpace: 'pre-wrap' }}>
                {msg.text === '' ? '...' : msg.text}
              </p>
            </div>
          </div>
      ...
      <form onSubmit={handleSubmit} className="mt-4 flex items-center gap-2">
        ...
        />
        <Button type="submit" disabled={isLoading || !input.trim()} className="!p-3 !rounded-xl" aria-label="Send message">
